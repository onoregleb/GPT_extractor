...
def generate_response(uploaded_file, employees_info, project_name, num_employees):
    if uploaded_file is not None:
        ...
        1.Найти в интернете контакты наиболее подходящих бизнес-партнеров. Узнать их рейтинг их положение на рынке. Для компании выгодно связаться с партнерами, которые имеют большие перспективы, 
        опыт в данной сфере и высокое положение над конкурентами. Партнер должен соответствовать информации об компании, приведенной в файле и выбор должен быть основан на специальности и продукта компании партнера.
        ...
        question20 = """Представь, что ты технический директор и программист по совместительству в новой компании, которая только собирается открываться и собирается выводить на рынок новый продукт. Информация о продукте содержится в файле, который я тебе прислал. Твоя задача - спрогнозировать техническое оснащение, которое необходимо приобрести в течении ближайшего года для разработки нашей программы. Необходимо обосновать выбор того или иного прибора, устройства, программы и т.д., также указывается стоимость. Пытайся сэкономить на лицензионных программах, приборах. К примеру, для программирования ты можешь написать, что для разработки мы будем использовать среду  PyCharm Community там какого-нибудь года, к примеру, 21го и мы ведь можем использовать эту IDE взломанную, т.к. ничего не будет (если нет, поправь меня и тогда будем везде использовать лицензионное ПО). 
    Если недостаточно информации из файла, используй свой опыт и знания, чтобы раскрыть этот раздел. """

        question21 = """Представь, ты маркетолог новой (СМО) компании, которая собирается открыться. 
        Информация о продукте и компании содержится в файле. Твоя задача провести анализ и сделать стратегию на основе
        примерных данных из файла. Изучить информацию о компании (валовой продукция, остатки на начало года и конец) и 
        подсчитать объём реализации по формуле. Сформировать финансовый результат деятельности предприятия и анализируя 
        данные из интернета об востребованности продукта посчитать спрос на продукцию компании и эффективность её 
        производства. Также важно учесть бюджет компании, привести примерные значения для будущего развития, и количество сотрудников, которые занимаются поставкой, разработкой новых видов и улучшений продукции и так далее. """

        question22 = """Представь, ты финансовый директор. Информацию о продукте в файле, который я тебе прислал. 
        Твоя задача составить суммы доходов в месяц исходя из источников доходов компании, а также анализ движения 
        наличности. Главный источник доходов – продажа продукта. Также ты должен провести оценку. Вторая задача отразить
        как изменится примерная прибыль за год."""

        question23 = """Представь, что ты финансовый директор в новой компании, которая только собирается открываться и собирается выводить на рынок новый продукт. Информация о продукте содержится в файле, который я тебе прислал. Твоя задача - рассчитать примерный ежемесячный (с указанием годовой суммы) расход компании к моменту выхода компании на самоокупаемость, таким образом, чтобы срок не был больше двух лет после завершения гранта. 
        В обязательные расходы входят разделы ""Перечень планируемых работ - этап 1"" и ""Перечень планируемых работ - этап 2"". Учитывая 200 и 800 тысяч рублей, в сумме миллион, спроецируй расходы на ближайшие 3 года таким образом, чтобы компания могла выходить на самоокупаемость. Распиши доходы и расходы, до и во время выхода компании на самоокупаемость, выделяя ключевые отличия в доходах и расходах. 
        ...
        vectordb.persist()  # сохраним БД *test git add 1
        vectordb = Chroma(
            persist_directory=persist_directory,
        ...
        titles = ["Команда", "Аннотация к проекту", "Какой продукт или услуга будет продаваться",
                  ...
                  "Держатель проблемы", "Заделы и пути взаимодействия", "Оценка потенциала рынка", "Партнеры",
                  "Коллектив - через год", "Техническое оснащение - через год", "Объём реализации продукции", "Доходы",
                  "Расходы", "Планируемый период выхода предприятия на самоокупаемость",
                  "Перечень планируемых работ - этап 1", "Перечень планируемых работ - этап 2", "Коллектив - данный момент",
                  "Техническое оснащение - данный момент", "Партнеры (поставщики, продавцы) - переговоры на данный момент",
                  "Формирование коллектива - ближайшие 3 года", "Функционирование юридического лица - ближайшие 3 года",
        ...
        try:
            ...
            for question, result in zip(titles[:13] + titles[24:28], results_first_part):
